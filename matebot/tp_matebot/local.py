...
## TODO fazer um local/__init__.py estatico e botar os demais arquivos de local/
## no .gitignore
...
class local:
  ...
  def __init__(self, args):
    ...
    if 'bot' in args.keys():
      self.bot = args['bot']
    self.grupos_local = [user for user in self.config['users']['beta'] + 
      self.config['users']['gamma'] + self.config['users']['delta'] + 
      self.config['users']['epsilon'] if user < 0]
    self.usuarios_local = [user for user in self.config['users']['beta'] + 
      self.config['users']['gamma'] + self.config['users']['delta'] + 
      self.config['users']['epsilon'] if user > 0]
    self.grupos_debug = [user for user in self.config['users']['alpha'] 
      if user < 0]
    self.usuarios_debug = [user for user in self.config['users']['alpha'] 
      if user > 0]

  ## Verifica se tem uma publicação nova no blog e envia para o grupo de 
  ##  telegram
  ## Na verdade não existe este comando, mas se alguém escrever o código, 
  ##  poderia existir!
  ## Este é só um exemplo de como o MateBot pode ou poderia fazer ações 
  ##  automáticas
  def loop_blog(self):
    time.sleep(0.001)
...
#        self.bot.sendMessage(self.grupos_local[0], str(response['response']))
        self.bot.sendMessage(self.grupos_local[0], str(response['response']))
        shiva_1({
          'numero': self.config['totalvoice']['agenda']['numero_3'],
          'config': self.config,
          'bot': self.bot,
